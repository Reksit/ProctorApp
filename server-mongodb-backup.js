...
app.get('/api/quizzes/today', authenticateToken, async (req, res) => {
  try {
    ...
    let quizzes = await Quiz.find({ isActive: true }).sort({ assignedDate: 1, startTime: 1 });
    ...
    const quizList = await Promise.all(quizzes.map(async (quiz) => {
      ...
      const cleanStart = formatHHMM(quiz.startTime, '00:00');
      const cleanEnd = formatHHMM(quiz.endTime, '23:59');

      if (quiz.assignedDate < todayStr) {
        isLocked = true;
        lockReason = `Exam window expired. This quiz was assigned for ${quiz.assignedDate}.`;
      } else if (quiz.assignedDate > todayStr) {
        isLocked = true;
        lockReason = `Exam window has not opened yet. Scheduled for ${quiz.assignedDate}.`;
      } else {
...
app.get('/api/quizzes/:id/take', authenticateToken, async (req, res) => {
  try {
    ...
    if (quiz.assignedDate < todayStr) {
      return res.status(400).json({ message: `Access Blocked: Quiz expired on ${quiz.assignedDate}.` });
    }
    if (quiz.assignedDate > todayStr) {
      return res.status(400).json({ message: `Access Blocked: Quiz scheduled for ${quiz.assignedDate}.` });
    }
    if (currentTimeStr < cleanStart) {
...
app.post('/api/admin/quizzes', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const newQuiz = new Quiz({
      ...
      assignedDate: assignedDate || getLocalDateString(),
